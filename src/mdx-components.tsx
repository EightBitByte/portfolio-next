...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    p: ({children}) => (
      <p className="mb-2 text-lg leading-8 dark:text-zinc-300">
        {children}
      </p>
    ),
    a: ({children, href}) => (
      <a href={href} 
         className="underline font-bold dark:text-zinc-200">
          {children}
      </a>
    ),
    blockquote: ({children}) => (
    ...
    ol: ({children}) => (
      <ol className="pl-8 dark:text-zinc-200">
        {children}
      </ol>
    ),
    li: ({children}) => (
    ...
    code: ({children}) => (
      <code className="text-md dark:text-zinc-200">
        {children}
      </code>
    ),
    ...components,
