...
export const ACHIEVEMENT_DATA: { [key: string]: AchievementInfo } = {
  ...
  HOW_DID_WE_GET_HERE: {
    ...
    desc: "Visit the 404 page.",
    flavor: "Did you find a broken link..?",
    points: 10,
...
const checkForCheating = (state: GameState): boolean => {
  const numUnlocked = Object.values(state.unlockedAchievements).filter(
    Boolean,
  ).length;
  const numTotal = Object.keys(ACHIEVEMENT_DATA).filter(
    (k) => !ACHIEVEMENT_DATA[k].secret,
  ).length;
  ...
  persist(
    (set, get) => ({
      ...
      unlockAchievement: (id) => {
        ...
        if (
          currentState.points > 175 &&
          Object.values(currentState.purchasedItems).every((p) => !p)
        ) {
          get().unlockAchievement("DELAYED_GRATIFICATION");
        }

        // Check Completionist
        const numUnlocked = Object.values(
          currentState.unlockedAchievements,
        ).filter(Boolean).length;
        const numTotal = Object.values(ACHIEVEMENT_DATA).filter(
          (a) => !a.secret,
        ).length;
      ...
      markBlogPostAsRead: (postId) => {
        ...
        if (
          postId < 0 ||
          postId >= state.achievementProgress.blogPostsRead.length
        )
          return;
        ...
        if (
          achievementProgress.numPosts > 0 &&
          readCount === achievementProgress.numPosts
        ) {
      ...
      purchaseItem: (id) => {
        ...
        const currentState = get();
        const numPurchased = Object.values(currentState.purchasedItems).filter(
          Boolean,
        ).length;
        const totalItems = Object.keys(SHOP_DATA).length;

        if (numPurchased > 0) currentState.unlockAchievement("LIGHT_SHOPPING");
        if (numPurchased === totalItems)
          currentState.unlockAchievement("BIG_SPENDER");
